...
//    }
    ...
    public static double addSubiect(double totalScore, double addedScore) {
...
//        System.out.println(addSubiect(4.5,3.5));
//        System.out.println(addSubiect(4.5,3.5)/ 2);
//
//        System.out.println(calculateAvg(addSubiect(4.5,3.5), 2));

        // 이상의코드를 확인하게 했을 때 알 수 있는 점은 addSubject() method의 결과값이 calculateAvg() method의
        // 첫 번째 argument가 됐다는 점입니다. -> 프로그래머스나 맥준에서 코테 연습하는 문제를 풀이에서
        // 이렇게 징그럽게 쓰는 사람들이 있습니다.

//        double sumOfScores = addSubiect(4.5,3.5);
//        int subjects = 2;
//        double avgScore = calculateAvg(sumOfScores, subjects);
//
        // 이상의 코드는 일단 두 과목을 기준으로 합과 편균을 낸다는 점을 확인할 수 있습니다.
        // method로 두 과목부터 100과목까지으 overloading을 통한 매개변수 늘리기 보다는
        ...
        double score1 = 0;
        double totalScore = 0;
        int totalSubjects = 1;  // 과목수가 될것이며 반복 횟수마다 ++시켜줄 예정(0으로 나누지 않기 위해 1로 초기화)
        double avgScire = 0;    // totalScore/totalSubjects의 결과값이 대입될거니까 미리 double로 선언함

        // 점수 입력을 횟수와 상관없이 받기 위한 반복문 작성
        while (!endOfCalc) {
            System.out.print("점수를 입력하세요(종료하려면 -1을 입력하세요) >>>> ");
            score1 = scanner.nextDouble();
            // score1 - -1이 대입되었다면 반복문이 종료될수 있도록 하는 조건문 작성
            if (score1 == -1) {     // 왜 '종료' 나 'x'가 아닌 -1을 입력했을 때 종료되도록 작성해야할까?
                break;
//                endOfClac = true   // 현재 반복은 이루어지고 다음 반복시에 종료가 되기 때문
                // 이코드 밑에 있는 모든 연산이 이루어 진다는 점에서 평균 계산이 제대로 되지 않음.
            }
            // 답을 계산하는 코드
            totalScore = addSubiect(totalScore, score1); // 얘의 의미는 첫 반복문일 때는 addSubiect(0, score1); 이겠습니다.

            //평균을 계산하는 코드
            avgScire = calculateAvg(totalScore, totalSubjects);
            System.out.println("----점수 입력시마다 나오는 합계와 평균입니다.-------");
            System.out.println("입력된 점수의 합계 : " + totalScore);
            System.out.println("입력된 점수의 평균 : " + avgScire);

            totalSubjects++;    // 다음 반복시에는 한 과목 더 늘어난 상태로 평균 계산의 분모가 되어야 하기 때문
